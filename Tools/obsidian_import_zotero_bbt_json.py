...
def obsidian_import_zotero_bbt_json(
    ...
    if create_review_note:
        review_background = _review_background(
            topic=resolved_topic,
        ...
        review_overview = _topic_overview(
            topic=resolved_topic,
            category=resolved_category,
            imported_count=len(imported_notes),
            type_counter=type_counter,
        ...
        review_metadata = {
            ...
            "keywords": [resolved_topic, "zotero-import", "materials-review"],
            "summary": review_background,
            "introduction": review_overview,
            "technical_foundations": "待基于已导入文献补写该主题的技术基础与发展脉络。",
            "main_research_directions": "待基于 literature notes 归纳主要研究方向与方法族谱。",
            "representative_comparison": "待围绕代表性工作补写任务设定、方法路线与结论差异。",
            "challenges": "待结合全文证据补写当前主题的关键挑战、争议与证据缺口。",
            "future_trends": "\n".join(f"- {step}" for step in next_review_steps),
            "conclusion": "待在完成证据整理与比较分析后补写结论。",
            "topic_overview": review_overview,
        ...
        if not review_result.get("ok"):
            return {
                ...
                **_vault_result_fields(
                    review_result,
                    requested_default=bootstrap_result.get("requested_vault_dir", str(vault_dir or "").strip()),
                    effective_default=bootstrap_result.get("effective_vault_dir", ""),
                    source_default=bootstrap_result.get("effective_vault_source", ""),
                ),
    ...
    final_vault_result = review_result if isinstance(review_result, dict) else bootstrap_result
    return {
        "ok": True,
        "export_json_path": str(export_path),
        "topic": resolved_topic,
        ...
        **_vault_result_fields(
            final_vault_result,
            requested_default=bootstrap_result.get("requested_vault_dir", str(vault_dir or "").strip()),
            effective_default=bootstrap_result.get("effective_vault_dir", bootstrap_result.get("vault_dir", "")),
            source_default=bootstrap_result.get("effective_vault_source", bootstrap_result.get("vault_source", "")),
        ),

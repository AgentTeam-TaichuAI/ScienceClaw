...
def _normalized_path_display(raw_path: str) -> str:
    key = _normalized_path_key(raw_path)
    if not key:
        return ""
    _prefix, _sep, value = key.partition(":")
    return value
...
def _vault_report(requested_vault_dir: str, effective_vault_dir: str, vault_source: str) -> dict[str, Any]:
    ...
    requested_normalized = _normalized_path_display(requested)
    effective_normalized = _normalized_path_display(effective)
    return {
        ...
        "requested_vault_match_success": match_status in {"exact", "normalized_same_path"},
        "requested_vault_dir_normalized": requested_normalized,
        "effective_vault_dir_normalized": effective_normalized,
    }
...
def _frontmatter(
    ...
    return yaml.safe_dump(frontmatter, allow_unicode=True, sort_keys=False, width=100000).strip()
